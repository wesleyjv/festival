...
class CartService
{
    ...
    /**
     * Simple named tickets (e.g. from standalone event pages) without a date slot.
     */
    public function addAdHocPricedTicket(array $post, int $quantity): bool
    {
        $ticketName = $post['ticket_name'] ?? null;

        if (empty($ticketName) && isset($post['ticket'])) {
            $parts = explode('|', (string) $post['ticket'], 2);
            $ticketName = trim($parts[0] ?? '');
        }

        if (!is_string($ticketName) || $ticketName === '') {
            return false;
        }
        ...
        if (!isset($adHocPrices[$ticketName])) {
            return false;
        }
